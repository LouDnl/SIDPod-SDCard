...
static spi_t spis[] = {  // One for each SPI.
    {
        .hw_inst = spi0,  // SPI component  // spi1
        .miso_gpio = 16,  // GPIO number (not pin number) // 12 RX
        .mosi_gpio = 19,  // 15 TX
        .sck_gpio = 18,   // 14 SCK
        ...
        // .dma_isr = spi_dma_isr
        .DMA_IRQ_num = DMA_IRQ_1  // 0 is default -> this setting is never used!?
    }
};
...
static sd_card_t sd_cards[] = {  // One for each SD card
    {
        ...
        .ss_gpio = 17,    // The SPI slave select GPIO for this SD card  // 9 CS
        .use_card_detect = false,
        // .card_detect_gpio = 17,   // Card detect  // 13 CS
        .card_detected_true = -1  // What the GPIO read returns when a card is
                                 // present. Use -1 if there is no card detect.
    }};

// void spi_dma_isr() { spi_irq_handler_1(&spis[0]); }
